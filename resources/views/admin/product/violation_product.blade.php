...
@section('content')
    <div class="count-product">
        <a class="c-pro-all">Sản phẩm vi phạm</a>
    </div>
    @include('admin.product.partials.search-box', ['action' => route('admin.product.all_product'), 'categories' => $categories])
    <div class="widget">
        <div class="responsive-table">
            <table cellpadding="0" cellspacing="0" width="100%" class="table-order-title sTable withCheck mTable" id="checkAll">
                ...
                @foreach($products as $product)
                    <tr>
                        <td style="text-align: center;"><input type="checkbox" name="" id="product{{ $product->id }}" value="{{ $product->id }}"></td>
                        <td>{{ $product->name }}</td>
